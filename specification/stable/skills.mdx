...
- **One registry, keyed by identity.** A host assembles a single skill registry from every origin it supports (filesystem skill directories, and `skills/list` results from each connected server declaring the extension), keyed by origin and `SKILL.md` URI together rather than by name. Assembling the registry reads only entries, and no `SKILL.md` or supporting file is fetched at this stage.
...
SDKs **SHOULD** provide thin wrappers in skill-specific terms.

- Server side: register a skill from a directory at a given skill path, populate resource metadata from frontmatter, compute entry digests and sizes, answer `skills/get` (and `skills/list` where the skill set is bounded), and warn when a registered skill exceeds the [Limits](#limits).
- Client side: `listSkills`, `getSkill`, a `resources/read` wrapper for skill URIs, and `readDirectory`.
