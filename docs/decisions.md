...
**Status:** Proposed — amended 2026-08-17: eager materialization is prohibited, so item 3's pinning primitive is gone and its atomic-snapshot claim now covers consistency only; see the virtual-filesystem mode entry below
...
### 2026-08-17: SEP-2640 v1 solves for the virtual-filesystem (read-through) mode

**Status:** Proposed

**Context:** Two host consumption patterns emerged across implementations of the draft:

- Read-through, where every skill read is a `resources/read` against the live server and no host filesystem is required.
- Materialization, where a host fetches, verifies, and installs a skill's files to local disk.

The outstanding core-maintainer concerns raised against the SEP in August centered on this split: the two patterns share a wire format but not a behavioral contract, which leaves conflicts between modes when resolving a skill's identity against a local file path rather than its remote origin, when executing scripts that not every host implementing the extension can support, and when setting quotas and capacity handling for servers serving many skills. After further discussion between core maintainers and WG leads, and a side-by-side review of the two patterns' value and cost, the group resolved to solve for the virtual-filesystem mode in v1 of this extension.

**Decision:** SEP-2640 v1 is specified for the virtual-filesystem mode:

1. **The solved-for consumption model is read-through.** A host consumes an MCP-served skill by reading its files on demand via `resources/read` (and, where declared, `resources/directory/read`) against the live server, verifying each read against the skill's `resources` manifest. This works on hosts with no filesystem, such as web, mobile, and enterprise-locked clients. The supporting rules landed on the SEP branch between 2026-08-11 and 2026-08-22 — on-demand retrieval, cache rules, identity across materialization, the activation window and route are all normative now — but nothing in the SEP names the two modes or declares one solved-for. That naming is this entry's job rather than the spec's, and with a single consumption model specified, profile equivalence and mode advertisement no longer need answering.
2. **Eager materialization is prohibited; on-demand fetch with caching is the model.** Landed in [`72cc599`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/72cc599d9319d044f70f8d88df5c5c1a0728e44c) and [`a983432`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/a9834326e77fe7c79bc8a1dc7f8871ae8ff85bf2). The Integrity section's permissive clause — "fetch, verify, and cache the entire set at approval time and serve every subsequent read from the verified copy" — is gone, replaced by a prohibition on retrieval ahead of need, "not on connection, not on listing, and not at approval". `SKILL.md` is fetched when a skill is loaded and a supporting file when it is read. Caching what has been fetched is endorsed and the existing cache rules govern it: digest comparison for staleness, cache isolation, and durable MCP origin. The Security Implications cache bullet now also requires a disk cache to be host-write-only and never modified in place, or re-hashed from the cached bytes on every access; a stored digest label or a modification time is not verification. Install-mode machinery — durable cache layout, disk quotas, URI-to-path mapping mechanics — stays out of the SEP, and a host feeding cached copies into filesystem skill machinery remains bound by read-equivalence (2026-04-19) and the cache-isolation rules.
3. **Selected clarifications from the concerns document are adopted alongside.** Landed: `size` metadata and per-skill limits ([`9408347`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/9408347ed8900b527dba55c5953f36f2282b6b21), as fixed host floors of 512 resources and 16 MiB rather than the advisory numbers originally proposed); the general statement that reading a skill file is not activating the skill ([`ccb6c3e`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/ccb6c3e31537bfedc5245c7aa9ce045bee573fb7)); the directory-listing cross-reference ([`641d1eb`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/641d1eb400e7f479f4dac3da177d8bc8fa9b4736)); and a definition of the window in which a host is acting on a skill ([`17b9412`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/17b94128a4f83c48e26c0eac170cf1eed5d0a449)). Not landed: the nesting enforcement-strategy statement, and the display-name half of the alias-instability warning — the SEP now requires durable state to bind to `(server, uri)`, but nothing says a disambiguated name is host-local UI state. Both are one-paragraph edits with author answers already on the concerns document, so they are transcription rather than drafting. The nesting rationale is recorded in [`docs/rationale.md`](rationale.md) via [#124](https://github.com/modelcontextprotocol/experimental-ext-skills/pull/124); only the SEP sentence is missing.
4. **Skill identity resolves live.** In read-through mode every load and every supporting-file read is an act of resolution against the server, with three requirements. (a) `(server, uri)` is a skill's canonical identity everywhere — registry, approvals, cache — and cached or materialized paths are aliases, never identities; landed in [`4b0368a`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/4b0368a25d4767dba8842e918aacb407b11e0a8c), which also requires a materialized path to encode the server identity so the origin is recoverable from the path. (b) A minimum host resolution behavior for loading a skill given only its URI — `skills/get`, verify, approve, load; the model hands the host a URI and hosts do not scan prose — is still open: the SEP's MUST has no mechanism behind it that is not marked illustrative, and the concerns document asks that it be made precise without specifying the mechanism, so this sequence should not go into the spec verbatim. (c) Reading a skill file never activates the skill, stated generally; landed in [`ccb6c3e`](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2640/commits/ccb6c3e31537bfedc5245c7aa9ce045bee573fb7).

**Rationale:** Picking one solved-for mode answers the central structural concern by making the read-through picture the specified one. Read-through is also the mode best solved with MCP rather than with packaging or snapshot alternatives.

**Open items this decision does not settle:**

- Whether an explicit user action — "install this skill for offline use" — counts as "need" under the prefetch prohibition. The SEP text does not say. As written the prohibition makes non-conforming what FastAgent and the ChatGPT plugin import flow do today, so this needs deciding here rather than left to implementations.
- Where the URI-to-path normalization-collision rule lives. Keeping mapping mechanics out of the SEP was deliberate, but case- and Unicode-normalization collisions are then unaddressed anywhere, and the SEP is not neutral on whether they matter: it lists those same collisions among the reasons to defer archive distribution. The deferral rationale and the sanctioned materialization path read as inconsistent until this is placed somewhere.

**Consequences for documents outside the SEP:** [`docs/rationale.md`](rationale.md) was brought in line with the landed text in [#124](https://github.com/modelcontextprotocol/experimental-ext-skills/pull/124), which re-syncs the repo's SEP copy to [`753b9f2`](https://github.com/modelcontextprotocol/modelcontextprotocol/blob/753b9f2be43e07fdd070e535d75f190cff14beea/seps/2640-skills-extension.md), records why an enclosing skill's manifest includes its nested skills, and records why hosts retrieve files on demand. [`docs/threat-model.md`](threat-model.md) needs a revision pass: it ranks `install` as the "recommended baseline", which the landed text contradicts outright. The same pass should resolve the fixture that claimed serve-from-verified-copy was a MUST, and decide where the URI-to-path normalization-collision rule from its Appendix A lives now that the SEP declined to carry mapping mechanics.

**References:**
- [TODO: 2026-08-17 meeting notes link]
- "Skills Over MCP — What we're solving for with SEP-2640" slides, presented 2026-08-17 [TODO: link or commit the deck to the repo]
- Core-maintainer concerns document [TODO: link], §1 "Two host profiles share a wire format but not a behavioral contract"
- [June 30, 2026 WG meeting notes](https://github.com/modelcontextprotocol/modelcontextprotocol/discussions/2994) — the "pick one" discussion.
- 2026-04-19 "Filesystem is a host-side implementation detail" entry above — the floor this decision confirms.
