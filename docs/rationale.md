...
The reason is server load. A listing may be arbitrarily large — these are the same catalogs that make an empty or partial `skills/list` legitimate ([Why May the Listing Be Empty or Partial?](#why-may-the-listing-be-empty-or-partial)) — and one skill may carry up to 512 files and 16 MiB ([Limits](sep-draft-skills-extension.md#limits)). A host that staged every skill it might use would put load on a server proportional to the catalog rather than to what a session reads, and every connecting host would repeat it. The same argument bounds the cost of deferring archives: a round trip per file scales with the files a session opens, not with the size of the skill.

Retrieving late does not weaken content-bound approval. Approval binds to the entry's `resources` set, not to bytes the host is holding, so a file fetched long after approval is verified against the set that was approved, and one that no longer matches fails verification when it is read.

What it gives up is the single verification point. Staging the whole set at approval and freezing it leaves no later read that can diverge from what was verified; verifying per read keeps that window open for the session. Two rules narrow the window without closing it. While a host is acting on a skill it may resolve reads only to URIs in the held entry, so an unlisted file is a verification failure rather than a fetch. And a disk cache must be host-write-only and never modified in place, or re-hashed from the cached bytes on every access — a stored digest label or a modification time is not verification — so a cache the model or its scripts can write cannot serve content that was never checked. What remains is a server returning different bytes for the same URI on a later read, which the [threat model](threat-model.md) tracks as content rotation and which per-read verification rejects.
