...
- **Domain-level discovery:** The [Agent Skills Discovery RFC](https://github.com/cloudflare/agent-skills-discovery-rfc) proposes `/.well-known/skills/` for organizations to publish skills at predictable URLs with content integrity (SHA-256 digests). The [agentskills well-known URI spec](https://github.com/agentskills/agentskills/pull/254) formalizes this further using `/.well-known/agent-skills/`, adding formal distribution types (`skill-md` and `archive`), archive safety requirements, and full HTTP client implementation steps. Both are complementary to MCP — they handle discovery and distribution while MCP handles runtime consumption.
