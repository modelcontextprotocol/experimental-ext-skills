...
- **Agent Plugins:** [agent-plugins.org](https://agent-plugins.org) ([agentplugins/agent-plugins-spec](https://github.com/agentplugins/agent-plugins-spec)) — Vendor-neutral format for packaging skills and MCP servers into a distributable plugin: a root `plugin.json` manifest, skills under `skills/<name>/SKILL.md`, and MCP configuration in `mcp.json`. Version 1.0.0 was published Jul 2026; 1.1.0 is an unreleased working draft. See [agent-plugins-analysis.md](agent-plugins-analysis.md) for how it compares to this WG's approach
...
- **AI Catalog (AI Card)** ([ai-catalog.io](https://ai-catalog.io/specification/), [Agent-Card/ai-catalog](https://github.com/Agent-Card/ai-catalog)) — Common standard for describing and discovering AI capabilities (A2A agent cards, MCP servers, skills) as catalog entries. Raised in the [Aug 25 2026 WG meeting](https://github.com/modelcontextprotocol/modelcontextprotocol/discussions/3307) §7: an official per-file digest needs a well-defined algorithm for file iteration and byte order, which is why the well-known URI and manifest digest the archive instead — and Agent Plugins is counting on the AI Catalog spec as the manifest that carries that digest. Its [trust manifest threat model](https://github.com/Agent-Card/ai-catalog/blob/main/specification/trust-manifest-threat-model.md) and the trust-manifest ADRs (0009, 0019, 0020) are close prior art for [our own threat model](threat-model.md). Decisions are recorded as numbered ADRs under [`adr/`](https://github.com/Agent-Card/ai-catalog/tree/main/adr), several of which [#126](https://github.com/modelcontextprotocol/experimental-ext-skills/issues/126) already cites as prior art on frontmatter design: ADR-0012 (closed core schema with vendor properties in a `metadata` object, since replaced by `extensions` in ADR-0017), ADR-0002 (deferring dependency expression between entries, blocked on AND vs OR semantics), and ADR-0015 (`urn:air:{publisher}:{namespace}:{name}` identifiers anchored on a publisher domain).
