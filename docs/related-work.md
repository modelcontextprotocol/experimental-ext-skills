...
| [Well-Known URI for Agent Skills](https://github.com/agentskills/agentskills/pull/254) | agentskills | Formal `/.well-known/agent-skills/` URI spec built on the Cloudflare RFC; defines `skill-md` and `archive` distribution types, `$schema` versioning, archive safety requirements, and full HTTP client implementation steps |
