...
This example shows how a single server can layer **all four workaround patterns**
on top of the canonical resources approach, so every client gets the best
experience its capabilities allow.

## The Four Patterns

This server implements all four patterns simultaneously:

| # | Pattern | Primitives | Model discovers skills via | Client support needed |
|---|---------|-----------|---------------------------|----------------------|
| 1 | **Server Instructions** | Instructions + Resources | System prompt (auto-injected) | Instructions support |
| 2 | **Tool Description Catalog** | Tools + Resources | `skill` tool description with XML catalog | Tool listing (universal) |
| 3 | **Load Skill Tool** | Tools + Resources | `load_skill` tool description listing names | Basic tool support |
| 4 | **MCP Prompts** | Prompts + Resources | Prompt menu (`/skills`, `/skill-{name}`) | Prompts support |
...
| Tool Description Catalog | Works everywhere models see tools; on-demand loading | Description grows with skill count |
| Load Skill Tool | Minimal description; simple contract | No structured descriptions for matching |
...
3. **Server instructions** — the `<available_skills>` XML (from
   `generateSkillsXML()`) is set as the server's `instructions` option,
   injected into the system prompt by supporting clients

4. **`skill` tool** — description embeds the full XML catalog; calling it with
   a skill name returns the SKILL.md content via `loadSkillContent()`

5. **`load_skill` tool** — simpler variant with just skill names in the
   description; same handler

6. **Prompts** — `/skills` lists all available skills; `/skill-{name}` returns
...
- Server instructions for broad, zero-effort compatibility
- A `skill` tool for model-controlled loading with rich descriptions
- A `load_skill` tool for minimal-overhead model-controlled loading
