...
| `type: "mcp-resource-template"` entries | `skill://docs/{product}/SKILL.md` | `listSkillTemplatesFromIndex()` |
| Supporting-file flow | resource template `skill://{+skillFilePath}` | `readSkillDocument()` |
| Per-skill manifest | `skill://<path>/_manifest` (SHA-256) | `readSkillManifest()` |
...
The client output walks through 7 sections, each demonstrating one SEP surface — index discovery, the three entry types, fallback enumeration, skill-md reads, archive fetch+unpack, and the supporting-file flow.
